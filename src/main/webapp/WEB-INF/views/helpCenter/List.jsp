		...
		<form action="ListSearch.do" method="get">
			<tr align="center">
				<td colspan="2">질문유형을 선택해주세요 <select name="kinds">
						...
						<option value="예매/관람">예매/예매/관람</option>
			...
			<table>
				...
				<c:forEach var="list" items="${list}">
					<tr align="center">
						<td width="60px">${list.no}</td>
						<td width="150px">${list.wdate}</td>
						<td width="600px"><a href="questionok.do?no=${list.no}&question_no=${list.no}">${list.title}</a></td>
						<td>${list.writer}</td>
						<td width="60px"><a href="answerok.do?question_no=${list.no}">${list.check_answer}</a></td>
						<td>${list.kinds}</td>
					</tr>
				</c:forEach>
			</table>
